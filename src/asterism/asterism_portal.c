...
int asterism_portal_parse_rule(const char *rule_str, struct asterism_portal_config_s *config)
{
    ...
    *second_hash = '\0';
    ...
    if (strstr(local_part, "://") == NULL) {
        snprintf(local_full, sizeof(local_full), "tcp://%s", local_part);
    } else {
        snprintf(local_full, sizeof(local_full), "%s", local_part);
    }
    ...
    if (strstr(remote_part, "://") == NULL) {
        snprintf(remote_full, sizeof(remote_full), "tcp://%s", remote_part);
    } else {
        snprintf(remote_full, sizeof(remote_full), "%s", remote_part);
    }

    if (asterism_parse_address(remote_full, &scheme, &host, &port, &host_type) != 0) {
        free(config->local_host);
        free(s);
        return -1;
    }
    ...
    if (strstr(relay_part, "://") == NULL) {
        snprintf(relay_full, sizeof(relay_full), "http://%s", relay_part);
    } else {
        snprintf(relay_full, sizeof(relay_full), "%s", relay_part);
    }
    ...
    if (asterism_parse_address(host_port_url, &scheme, &host, &port, &host_type) != 0) {
        free(config->local_host);
        free(config->remote_host);
        AS_SFREE(config->relay_user);
        AS_SFREE(config->relay_pass);
        free(s);
        return -1;
    }
...
static void on_relay_read(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
    ...
    char *hdr_end = strstr(relay_stream->buffer, "\r\n\r\n");
    if (hdr_end) {
        if (strstr(relay_stream->buffer, " 200 ") != NULL) {
...
static void on_relay_connected(uv_connect_t *req, int status)
{
    ...
    if (conn->portal->config->relay_user && strlen(conn->portal->config->relay_user) > 0) {
        char creds[512];
        snprintf(creds, sizeof(creds), "%s:%s", conn->portal->config->relay_user, conn->portal->config->relay_pass ? conn->portal->config->relay_pass : "");
        char b64_creds[1024];
        int b64_len = 0;
    ...
    uv_buf_t buf = uv_buf_init(req_buf, req_len);
    conn->handshake_write.data = conn;
    asterism_stream_write(&conn->handshake_write, &conn->relay_stream, &buf, on_handshake_write_complete);
}
