...
```mermaid
graph LR
    Client["客户端 (Client)<br>(浏览器 / Curl)"] ==>|HTTP / SOCKS5 代理| Relay["Asterism 中继 (Relay)<br>(公网 IP)"]
    Relay <==|反向隧道| Agent["Asterism 代理 (Agent)<br>(内网环境)"]
    Agent ==>|本地访问| Target["目标服务<br>(SSH / Web / NAS)"]
...
```mermaid
graph LR
    App["本地应用"] ==>|TCP 连接| Portal["Asterism 传送门 (Portal)<br>(本地监听)"]
    Portal ==>|HTTP CONNECT 隧道| Relay["Asterism 中继 (Relay)<br>(公网 IP)"]
    Relay <==|反向隧道| Agent["Asterism 代理 (Agent)<br>(内网环境)"]
